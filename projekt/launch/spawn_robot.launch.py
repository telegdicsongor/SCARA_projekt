...
def generate_launch_description():
    ...
    try:
        get_package_prefix('joint_state_broadcaster')
        has_joint_state_broadcaster = True
    ...
    controller_state_to_joint_states_node = Node(
        package='projekt',
        executable='controller_state_to_joint_states.py',
        name='controller_state_to_joint_states',
        output='screen',
        parameters=[
    ...
    arm_controller_after_spawn = RegisterEventHandler(
        OnProcessExit(
            target_action=spawn_urdf_node,
            on_exit=[joint_trajectory_controller_spawner],
        )
    ...
    if has_joint_state_broadcaster:
        ...
        launchDescriptionObject.add_action(
            LogInfo(
                msg='joint_state_broadcaster package not found; relaying arm_controller/controller_state to /joint_states'
            )
        )
